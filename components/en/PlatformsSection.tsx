const platforms = [
  {
    name: "WhatsApp",
    description:
      "Automatically reply to WhatsApp Business messages — FAQs, orders, tracking, and everything your customers need.",
    color: "#25D366",
    bgColor: "rgba(37,211,102,0.08)",
    borderColor: "rgba(37,211,102,0.25)",
    features: ["Text & media messages", "Smart interactive replies", "WhatsApp Business API integration"],
    icon: (
  ...
  },
  {
    name: "Instagram",
    description:
      "Auto-reply to Instagram DMs — turn inquiries into instant sales without any manual intervention.",
    color: "#E1306C",
    bgColor: "rgba(225,48,108,0.08)",
    borderColor: "rgba(225,48,108,0.25)",
    features: ["Direct messages (DM)", "Order registration", "Product data integration"],
    icon: (
      <svg className="w-8 h-8" fill="currentColor" viewBox="0 0 24 24">
  ...
  },
  {
    name: "Messenger",
    description:
      "Automate Facebook Messenger conversations — continuous customer service for your followers and Facebook store.",
    color: "#0084FF",
    bgColor: "rgba(0,132,255,0.08)",
    borderColor: "rgba(0,132,255,0.25)",
    features: ["Facebook messages", "Business page integration", "Context-aware smart replies"],
    icon: (
      <svg className="w-8 h-8" fill="currentColor" viewBox="0 0 24 24">
          ...
          {platforms.map((p) => (
            ...
            >
              ...
              <div
                ...
                style={{ background: p.bgColor, color: p.color, border: `1px solid ${p.borderColor}` }}
              >
                {p.icon}
              </div>

              {/* Name */}
              <h3 className="text-xl font-black text-[#1E1B4B] mb-3">{p.name}</h3>
              ...
              {/* Active badge */}
              <div
                className="absolute top-5 left-5 text-[10px] font-black px-2 py-0.5 rounded-full"
                style={{ background: p.bgColor, color: p.color, border: `1px solid ${p.borderColor}` }}
              >
                Available Now
              </div>
