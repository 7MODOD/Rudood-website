...
/* ── Feature cards data ── */
const features = [
  { icon: "⚡", bg: "rgba(6,182,212,0.10)",  border: "rgba(6,182,212,0.25)",  title: "Lightning Fast",      desc: "Replies in seconds, not minutes — significantly boosts your deal-closing rates." },
  { icon: "🧠", bg: "rgba(124,58,237,0.10)", border: "rgba(124,58,237,0.25)", title: "Context Aware",       desc: "Remembers prior conversations and replies in a logical sequence just like a human." },
  { icon: "👥", bg: "rgba(37,99,235,0.10)",  border: "rgba(37,99,235,0.25)",  title: "Instant Human Handoff", desc: "When needed, it alerts your team and transfers the conversation seamlessly without interruption." },
];

/* ══════════════════════════════════════════════ */
export default function TryNow() {
  const [msgs,    setMsgs]    = useState<Msg[]>([
    { from: "agent", text: "Hello! How can I help you today? 😊", time: "09:41" },
  ]);
  ...
  const initSession = async (): Promise<Session> => {
    ...
    const cRes  = await fetch(`${PUB}/contacts`, {
      ...
      body:    JSON.stringify({ name: "Website Visitor", identifier: visitorId }),
    });
  ...
  const send = async () => {
    ...
    try {
      ...
      setMsgs(prev => [
        ...prev,
        { from: "agent", text: "A connection error occurred, please try again.", time: clock() },
      ]);
    }
  };
