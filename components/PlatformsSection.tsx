const platforms = [
  {
    name: "واتساب",
    englishName: "WhatsApp",
    description:
      "الرد التلقائي على رسائل واتساب Business — الأسئلة الشائعة، الطلبات، التتبع، وكل ما يحتاجه عميلك.",
    color: "#25D366",
    bgColor: "rgba(37,211,102,0.08)",
    borderColor: "rgba(37,211,102,0.25)",
    features: ["رسائل النص والميديا", "قوائم تفاعلية", "ربط واتساب Business API"],
    icon: (
      <svg className="w-8 h-8" fill="currentColor" viewBox="0 0 24 24">
        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
      </svg>
    ),
  },
  {
    name: "إنستغرام",
    englishName: "Instagram",
    description:
      "رد تلقائي على DMs ورسائل إنستغرام — تحويل الاستفسارات إلى مبيعات فورية بدون تدخل يدوي.",
    color: "#E1306C",
    bgColor: "rgba(225,48,108,0.08)",
    borderColor: "rgba(225,48,108,0.25)",
    features: ["الرسائل المباشرة DM", "ردود على التعليقات", "ربط بيانات المنتجات"],
    icon: (
      <svg className="w-8 h-8" fill="currentColor" viewBox="0 0 24 24">
        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
      </svg>
    ),
  },
  {
    name: "ماسنجر",
    englishName: "Messenger",
    description:
      "أتمتة محادثات ماسنجر على فيسبوك — خدمة عملاء متواصلة لمتابعيك ومتجرك على فيسبوك.",
    color: "#0084FF",
    bgColor: "rgba(0,132,255,0.08)",
    borderColor: "rgba(0,132,255,0.25)",
    features: ["رسائل فيسبوك", "ربط صفحة الأعمال", "ردود ذكية بالسياق"],
    icon: (
      <svg className="w-8 h-8" fill="currentColor" viewBox="0 0 24 24">
        <path d="M12 0C5.373 0 0 4.974 0 11.111c0 3.498 1.744 6.614 4.469 8.654V24l4.088-2.242c1.092.3 2.246.464 3.443.464 6.627 0 12-4.975 12-11.111S18.627 0 12 0zm1.191 14.963l-3.055-3.26-5.963 3.26L10.732 8.1l3.13 3.26L19.752 8.1l-6.561 6.863z" />
      </svg>
    ),
  },
];

export default function PlatformsSection() {
  return (
    <section id="platforms" className="py-24 bg-night">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Section Header */}
        <div className="text-center mb-16">
          <p className="text-cyan text-sm font-semibold uppercase tracking-widest mb-3">
            المنصات المدعومة
          </p>
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black text-text-primary mb-4">
            أينما كان عميلك،{" "}
            <span className="text-gradient">ردود موجود</span>
          </h2>
          <p className="text-text-muted text-lg max-w-2xl mx-auto">
            منصة واحدة تربط كل قنوات التواصل مع عملاءك وتجعل الردود الذكية في كل مكان.
          </p>
        </div>

        {/* Platform Cards */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {platforms.map((p) => (
            <div
              key={p.name}
              className="relative p-7 rounded-3xl border transition-all duration-300 hover:scale-[1.02] group"
              style={{
                background: p.bgColor,
                borderColor: p.borderColor,
              }}
            >
              {/* Icon */}
              <div
                className="w-14 h-14 rounded-2xl flex items-center justify-center mb-5"
                style={{ background: p.bgColor, color: p.color, border: `1px solid ${p.borderColor}` }}
              >
                {p.icon}
              </div>

              {/* Names */}
              <h3 className="text-xl font-black text-text-primary mb-1">{p.name}</h3>
              <p className="text-xs font-medium text-text-faint mb-3">{p.englishName}</p>

              {/* Description */}
              <p className="text-text-muted text-sm leading-relaxed mb-5">{p.description}</p>

              {/* Features */}
              <div className="space-y-2">
                {p.features.map((f) => (
                  <div key={f} className="flex items-center gap-2">
                    <svg
                      className="w-3.5 h-3.5 shrink-0"
                      style={{ color: p.color }}
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                    >
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" />
                    </svg>
                    <span className="text-text-muted text-xs">{f}</span>
                  </div>
                ))}
              </div>

              {/* Coming soon / active badge */}
              <div
                className="absolute top-5 start-5 text-[10px] font-black px-2 py-0.5 rounded-full"
                style={{ background: p.bgColor, color: p.color, border: `1px solid ${p.borderColor}` }}
              >
                متاح الآن
              </div>
            </div>
          ))}
        </div>

        {/* Bottom note */}
        <div className="mt-10 text-center">
          <p className="text-text-faint text-sm">
            المزيد من المنصات قادمة قريباً · تيليغرام · تيك توك · سناب شات
          </p>
        </div>
      </div>
    </section>
  );
}
